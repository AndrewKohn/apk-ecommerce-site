...
const TestimonialCard = props => {
  ...
  const getRandomTestimonialValue = array => {
    return Math.floor(Math.random() * array.length);
  };

  return (
    <div className={classes[`testimonial-card`]}>
      <span>{names[getRandomTestimonialValue(names)]}</span>
      <span>{dates[getRandomTestimonialValue(dates)]}</span>
      <span>
        {testimonialTitles[getRandomTestimonialValue(testimonialTitles)]}
      </span>
      <span>
        {testimonialTexts[getRandomTestimonialValue(testimonialTexts)]}
      </span>
    </div>
  );
};
